...
public class TronNeonBikeBattle extends JFrame {
    ...
    public TronNeonBikeBattle() {
        ...
        addKeyListener(new KeyAdapter() {
            ...
            public void keyPressed(KeyEvent e) {
                ...
                if ((player1 != null ) && player1Keys.contains(keyCode)){
                    handlePlayer1KeyPress(keyCode);
                }
                if ((player2 != null ) && player2Keys.contains(keyCode)){
    ...
    private void startNewGame() {
        ...
        player1 = new Player(player1Name, player1Color, player1TrailColor, WIDTH / 4, HEIGHT / 2, CELL_SIZE);
        player2 = new Player(player2Name, player2Color, player2TrailColor, 3 * WIDTH / 4, HEIGHT / 2, CELL_SIZE);
    ...
    private class GameLoop implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            if (collisionres != CollisionResultEnum.NOONECOLLIDED){
                String winner = (collisionres == CollisionResultEnum.PLAYER1COLLIDED ? player2.getName() : player1.getName());
                updateScore(winner);
                gameTimer.stop();
                displayTimer.stop();
    ...
    private void drawTrails(Graphics g) {
        ...
        for (int i = 1; i < player1.getTurningPointsSize(); i++) {
            Point start = player1.getElementOfTurningPoints(i - 1);
            Point end = player1.getElementOfTurningPoints(i);
            g2d.drawLine(start.x, start.y, end.x, end.y);
        }
        //System.out.println("Len of tp " + player2.turningPoints.size());
        // Draw Player 2's trail
        g2d.setColor(player2.getTrailColor());
        for (int i = 1; i < player2.getTurningPointsSize(); i++) {
            Point start = player2.getElementOfTurningPoints(i - 1);
            Point end = player2.getElementOfTurningPoints(i);
            g2d.drawLine(start.x, start.y, end.x, end.y);
        }
    }
    ...
    private void drawPlayers(Graphics g) {
        ...
        g.fillRect(player1.getCurrentPositionX(), player1.getCurrentPositionY(), CELL_SIZE, CELL_SIZE);

        g.setColor(player2.getColor());
        g.fillRect(player2.getCurrentPositionX(), player2.getCurrentPositionY(), CELL_SIZE, CELL_SIZE);
    }
    ...
    public void paintJustOneSegment(Graphics g) {
        ...
        if (running) {
            ...
            g.fillRect(player1.getPrevPosition().x, player1.getPrevPosition().y, CELL_SIZE, CELL_SIZE);

            // Draw the new segments of the trail for player 2
            g.setColor(player2.getTrailColor());
            g.fillRect(player2.getPrevPosition().x, player2.getPrevPosition().y, CELL_SIZE, CELL_SIZE);
    ...
    public void drawPlayer(Graphics g, Player player){
        ...
        g.fillRect(player.getCurrentPositionX(), player.getCurrentPositionY(), CELL_SIZE, CELL_SIZE);
    }

    void handlePlayer1KeyPress(Integer key) {
        int currentDx = player1.getDx();
        int currentDy = player1.getDy();
        player1.setDx(player1Keys.getDX(key, currentDx));
        player1.setDy(player1Keys.getDY(key, currentDy));
    }
    void handlePlayer2KeyPress(Integer key) {
        int currentDx = player2.getDx();
        int currentDy = player2.getDy();
        player2.setDx(player2Keys.getDX(key, currentDx));
        player2.setDy(player2Keys.getDY(key, currentDy));
    }
