...
public class Bag {
    ...
    public Bag intersection(Bag bag) {
        Bag result = new Bag();
        for (BagItem bi : bag.data) {
            if (this.contains(bi.getItem())) {
                result.add(bi.getItem(), Math.min(bi.getNum(),this.getItem(bi.getItem()).getNum()) );
            }
        }
        return result;
    }
    ...
    public Bag difference(Bag bag){
        Bag result = new Bag();
        Bag union = bag.union(this);
        Bag intersection = bag.intersection(this);
        for (BagItem bi : union.data) {
            if (!intersection.contains(bi.getItem())) {
                result.add(bi.getItem(), bi.getNum());
            }
        }
        return result;
