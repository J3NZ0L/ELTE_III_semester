...
function clickTile(e) {
    if(e.target.matches("td")) {
        ...
        if (checkForWin(row, col)) {
            ...
            table.removeEventListener("click", clickTitle)
        }
        else if(checkForTie()) {
...
function checkForWin(row, col) {
    return 5 < countInDirection(row, col, -1, -1) + countInDirection(row, col , 1, 1)
        || 5 < countInDirection(row, col, 1, -1) + countInDirection(row, col, -1, 1)
        || 5 < countInDirection(row, col, 1, 0) + countInDirection(row, col , -1, 0)
        || 5 < countInDirection(row, col, 0 ,1) + countInDirection(row, col, 0 , -1)
}
